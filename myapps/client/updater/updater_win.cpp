// myapps/client/updater/updater_win.cpp
        ...
        int       nShowCmd)
{
    ...
    auto thr = thread([](HWND hWnd, HWND hProgCtrl, int argc, LPWSTR* argv) {
		...
		for (int i = 0; i < 100; ++i) {
			...
			if (!running) break;
			if (i == 50) {
				SetWindowText(hWnd, TEXT("MyApps.directory update: check installer"));
			}
			if (i == 99) {
				SetWindowText(hWnd, TEXT("MyApps.directory update: start installer"));
			}
		}
...
#endif
        wstring des_path;
        SetWindowText(hWnd, TEXT("MyApps.directory update: copy installer"));
        ...
        SetWindowText(hWnd, TEXT("MyApps.directory update: validate installer"));
        ...
        SetWindowText(hWnd, TEXT("MyApps.directory update: launch installer"));
        //::SendNotifyMessage(hProgCtrl, PBM_SETPOS, (WPARAM)(INT)(0), 0);
