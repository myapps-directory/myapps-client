    ...
    FSP_FSCTL_VOLUME_INFO*                     VolumeInfo)
{
    ...
    if (!GetDiskFreeSpaceExW(Root, 0, &TotalSize, &FreeSize))
        return FspNtStatusFromWin32(GetLastError());
...
static FSP_FILE_SYSTEM_INTERFACE init_interface() {
    ...
    si.SetVolumeLabel    = SetVolumeLabel_;
    si.GetSecurityByName = GetSecurityByName;
    si.Create            = Create;
    si.Open              = Open;
    ...
    si.Write             = Write;
    si.Flush             = Flush;
    si.GetFileInfo       = GetFileInfo;
    si.SetBasicInfo      = SetBasicInfo;
    si.SetFileSize       = SetFileSize;
    si.Rename            = Rename;
    si.GetSecurity       = GetSecurity;
    si.SetSecurity       = SetSecurity;
    si.ReadDirectory     = ReadDirectory;
    si.SetDelete         = SetDelete;

    return si;
}

static FSP_FILE_SYSTEM_INTERFACE PtfsInterface = init_interface();
...
static NTSTATUS EnableBackupRestorePrivileges(VOID)
{
    ...
    if (!LookupPrivilegeValueW(0, L"" SE_BACKUP_NAME, &Privileges.P.Privileges[0].Luid) || !LookupPrivilegeValueW(0, L"" SE_RESTORE_NAME, &Privileges.P.Privileges[1].Luid))
        return FspNtStatusFromWin32(GetLastError());
